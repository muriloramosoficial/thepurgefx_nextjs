...
const DashboardContent = () => {
    ...
    useEffect(() => {
        ...
        if (card01!.offsetWidth < 300 || card02!.offsetWidth < 300 || card03!.offsetWidth < 300 || card04!.offsetWidth < 300) {
          setShowX(true);
        } else {
          setShowX(false);
        }
        setCardWidth(card01!.offsetWidth && card02!.offsetWidth && card03!.offsetWidth && card04!.offsetWidth);
      }, []);

    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-10">
            <div className="flex flex-row p-10 bg-slate-600 rounded-xl" id="card01">
                {showX ? <div>x</div> : <div>y</div>}
